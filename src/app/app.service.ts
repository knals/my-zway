...
export class AppService {
  ...
  public executeCommand(device: Devices, command: Command): Observable<any> {
    const url = `http://raspberrypi.local/ZAutomation/api/v1/devices/${device}/command/${command}`;
  ...
  public commandOn(device: string | 'ZWayVDev_zway_8-0-38'): Observable<any> {
    const url = `http://raspberrypi.local/ZAutomation/api/v1/devices/${device}/command/on`;
    return this.HttpClient.get<any>(url, {});

  }

  public commandOff(device: string | 'ZWayVDev_zway_8-0-38'): Observable<any> {
    const url = `http://raspberrypi.local/ZAutomation/api/v1/devices/ZWayVDev_zway_8-0-38/command/off`;
    return this.HttpClient.get<any>(url, {});

  }

  public commandStop(device: string | 'ZWayVDev_zway_8-0-38'): Observable<any> {
    const url = `http://raspberrypi.local/ZAutomation/api/v1/devices/ZWayVDev_zway_8-0-38/command/stop`;
    return this.HttpClient.get<any>(url, {});
